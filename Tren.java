...
public class Tren {
	...
	public void moverElTren(Estacion destino, boolean ida) {
		Tren tren = new Tren(id, linea, estacion, controlTrenes);
		List<Estacion> estaciones = linea.getEstaciones();
		/*if(linea.getId() == 6) {
			if(ida == false) {
				tren.irA(destino, estacion, !ida);
			} else {
				tren.irA(estacion, destino, ida);
			}	
		} else {
			if(controlTrenes.continuoDandoVueltas(tren) == true) { 
				tren.controlTrenes.setFinEjecucion(true);
			} else {
				while(!controlTrenes.continuoDandoVueltas(tren)) {
					if(destino == estaciones.get(0)) {
						ida = false;
					} else if(destino == estaciones.get(estaciones.size() -1))
						ida = true;
					tren.irA(estacion, destino, ida);
				}
			}
		}*/
		if(controlTrenes.continuoDandoVueltas(tren)) { 
			tren.irA(estacion, destino, ida);
			tren.controlTrenes.setFinEjecucion(true);
		}
		while(!controlTrenes.continuoDandoVueltas(tren)) {
		...
		if(ida == false) {
			destino = estaciones.get(0);
		} else if(ida = true)
			destino = estaciones.get(estaciones.size() -1);
		if(linea.getId() == 6) {
			if(ida == true) {
				tren.irA(estacion, destino, ida);
			} else {
